...
// buttons = Array.from(buttons).map(function(button, index) {
//   button.addEventListener('click', function() {
//       button.classList.add('bg')
//   });

//   return button; // Return the button to keep the mapped array
// });
    ...
    buttons[i].addEventListener("click", () => {
        ...
        for (let j = 0; j < buttons.length; j++) {
            if (i != j) {
                ...
                buttons[j].querySelector('.mountains-line').classList.remove('line-color')
            }
        }
    })
}